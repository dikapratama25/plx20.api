...
namespace Plexform.Migrations.Seed.Tenants
{
    public class TenantRoleAndUserBuilder
    {
        ...
        public TenantRoleAndUserBuilder(PlexformDbContext context, int tenantId)
        {
            _context = context;
            _tenantId = tenantId;
        }
        ...
        private void CreateRolesAndUsers()
        {
            ...
            var _public = "Public";
            var freePub = _context.Roles.IgnoreQueryFilters().FirstOrDefault(r => r.TenantId == _tenantId && r.Name == _public);
            if (freePub == null)
            {
                _context.Roles.Add(new Role(_tenantId, _public, _public) { IsStatic = true, IsDefault = true });
                _context.SaveChanges();
            }
            ...
            var _biz = "Biz";
            var freebiz = _context.Roles.IgnoreQueryFilters().FirstOrDefault(r => r.TenantId == _tenantId && r.Name == _biz);
            if (freePub == null)
            {
                _context.Roles.Add(new Role(_tenantId, _biz, _biz) { IsStatic = true, IsDefault = false });
                _context.SaveChanges();
            }
            ...
            if (adminUser == null)
            {
                adminUser = User.CreateTenantAdminUser(_tenantId, "info@collexe.com");
